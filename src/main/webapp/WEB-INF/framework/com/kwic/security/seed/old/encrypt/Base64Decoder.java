// Base64Decoder.java
// $Id: Base64Decoder.java,v 1.1 2012/05/21 00:42:33 nightbobo Exp $
 ...
 * @see com.kwic.security.seed.old.encrypt.tools.codec.Base64Encoder
 */
    ...
    /*public static void main (String args[]) {
		if ( args.length == 1 ) 
		{
			try 
			{
				...
				Debug.out ("["+b.processString()+"]") ;
			}
			catch (Base64FormatException e) 
			{
				Debug.out ("Invalid Base64 format !") ;
				System.exit (1) ;
			}
		} 
		else if (args.length == 2) 
		{
	    	...
	    	else
	    	{
				try 
				{
					...
					Base64Decoder b = new Base64Decoder(args[0], out);
					b.process();
	    		} 
	    		catch (Exception ex) 
	    		{
		...
		else if (args.length == 3) 
		{
			if( args[0].equals("-ff") )
	    	{
				try 
				{
					...
					Base64Decoder b = new Base64Decoder (in, out);
					b.process();
	    		} 
	    		catch (Exception ex) 
	    		{
	    		...
	    		Debug.out("Base64Decoder [string] [-f in-file] [string outfile] [-ff in-file outfile]");
	    }
	    else
	    {
			Debug.out("Base64Decoder [string] [-f in-file] [string outfile] [-ff in-file outfile]");

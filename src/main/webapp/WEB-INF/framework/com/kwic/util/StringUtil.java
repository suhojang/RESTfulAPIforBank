...
public class StringUtil {
	...
	*/
	...
	public static final String subChar(String str, int size, boolean left) {
		if(str==null)
			str	= "";

		if(size < 1 || str.getBytes().length < size)
			return str;
		...
		if(str.getBytes().length > size){
			...
			if (left) {
				...
				System.arraycopy(str.getBytes(), str.getBytes().length - bytes.length, bytes, 0, bytes.length);
			}
			
			sb.append(new String(bytes));
		}else if(str.getBytes().length==size){
			sb.append(str);
		}
		return sb.toString();
	}
	...
	public static Object nvl(Object str, String defaultVal) {
		return str == null ? defaultVal : str;
	}
	
	/**
	 * 문자열의 특정 자리수에 치환하는 문자열로 변환
	 * @param src		: 기본 문자열
	 * @param idx		: 변경 할 문자열 위치
	 * @param newstr	: 변경 할 문자열
	 * @return
	 */
	public static String replaceIndexStr(String src, int idx, String newStr) {
		if (src==null)
			return src;
		
		StringBuilder builder	= new StringBuilder(src);
		builder.setCharAt(idx, newStr.charAt(0));
